...
const CORS_CONFIG = Object.freeze({
  ...
  allowedOrigins: [
    ...
    // "https://myplug.com",
    // "https://www.myplug.com",
    // "https://app.myplug.com",
  ],
...
const MARKETS = Object.freeze([
  /* ── ACTIVE MARKETS ── */
  {
    ...
    hero: "https://images.unsplash.com/photo-1572981779307-38b8cabb2407?w=1600&q=80",
...
/* ── Session management (uses localStorage — not window.storage) ────────────── */
const SESSION_KEY = "plug_session";
function _saveSessionLocal(session) {
...
async function submitVendorApp(vendorId, app) {
  ...
  await sb.from("vendor_profiles").update({
...
async function updateRequestStatus(reqId, status, note = "") {
  ...
  const { data } = await sb.from("booking_requests").update({
...
async function recordAttempt(email, success) {
  try {
    ...
    /* Production: Supabase */
    if (success) {
      await sb.from("rate_limits").delete().eq("identifier", key);
      return;
    }
...
async function checkVerifyCode(userId, type, input) {
  ...
  await sb.from("verify_codes").update({ used: true }).eq("id", data.id);
  return "ok";
}
...
async function toggleFavorite(userId, vendorId) {
  ...
  if (current.includes(vendorId)) {
    await sb.from("favorites").delete().eq("user_id", userId).eq("vendor_id", vendorId);
...
function AuthModal({ onClose, onAuth }) {
  ...
  async function submitEmailVerify() {
    ...
    if (result === "ok") {
      ...
      if (IS_PREVIEW) {
        ...
        await sb.from("profiles").update({ email_verified: true }).eq("id", pendingUser.id);
      }
      /* Create session */
  ...
  function resendCode() {
    ...
    const code = genVerifyCode();
    storeVerifyCode(pendingUser.id, "email", code).then(() => {
      setDevCode(code);
      setVerifyCode("");
      setErr("");
    });
...
function AdminPanel({ user, onClose }) {
  const [atab,       setAtab]      = React.useState("vendors");
  const [vendorApps, setVendorApps]= React.useState([]);
  const [busy,       setBusy]      = React.useState(false);
  const origin = (typeof window !== "undefined" ? window.location.origin : "") || "null";
  const originOk = isOriginAllowed(origin);

  React.useEffect(() => { getVendorApps().then(setVendorApps); }, []);
...
export default function PlugApp() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (userId) {
        const u = await _findUserById(userId);
        if (u) setUser(sanitizeUser(u)); // _findUserById returns profile-only; sanitizeUser is belt-and-suspenders
      }
    })();
  }, []);
    ...
    <div className="plug">
      ...
      <nav className={`plug-nav${navScrolled ? " nav-glass" : ""}`}
        style={{
          ...
          padding:"0 28px",
          background: navOnHero ? "transparent" : "rgba(255,255,255,0.88)",
          borderBottom: `1px solid ${navOnHero ? "transparent" : "rgba(255,255,255,0.5)"}`,
          ...
          {CATEGORIES.map(cat => (
            <button key={cat.id} onClick={() => pickCat(cat.id)} className="btn"
              style={{ padding:"6px 14px", borderRadius:99, fontSize:12, fontWeight:600,
                       ...
                       background: activeCat===cat.id ? (navOnHero ? "#fff" : C.black) : "transparent",
                       color: activeCat===cat.id ? (navOnHero ? C.black : "#fff") : (navOnHero ? "rgba(255,255,255,0.8)" : C.midGray),
            ...
            <>
              ...
              <button onClick={() => { setAccountOpen(o=>!o); setNotifOpen(false); }}
                ...
                <span style={{ fontSize:12, fontWeight:700,
                               color: navOnHero || user.type==="admin" ? "#fff" : C.black }}>
      ...
      {isHero && (
        <div style={{ position:"relative", height:480, overflow:"hidden" }}>
          <img src={market?.hero || "https://images.unsplash.com/photo-1492684223066-81342ee5ff30?w=1600&q=80"} alt={market?.label || "Event"}
            style={{ width:"100%", height:"100%", objectFit:"cover", objectPosition:"center 30%" }} />
          <div style={{ position:"absolute", inset:0, background:"linear-gradient(105deg, rgba(0,0,0,0.78) 0%, rgba(0,0,0,0.45) 55%, rgba(0,0,0,0.15) 100%)" }} />
          <div style={{ position:"absolute", inset:0, display:"flex", alignItems:"center", padding:"0 7% 0 6%" }}>
            <div style={{ maxWidth:560 }} className="fade-up">
              ...
              {/* Search pill */}
              <div style={{ background:"#fff", borderRadius:14, padding:"6px 6px 6px 14px",
                            display:"flex", alignItems:"center", gap:8, maxWidth:520,
                            boxShadow:"0 8px 32px rgba(0,0,0,0.35)" }}>
                <Emoji e="📍" size={16} />
                <input placeholder="What are you looking for? Tacos, DJ, lighting..."
                  onClick={()=>searchRef.current?.focus()}
                  style={{ flex:1, border:"none", background:"transparent", fontSize:14, color:C.black,
                           fontWeight:500, height:38, outline:"none", fontFamily:"'Inter',sans-serif" }}
                  value={search} onChange={e=>setSearch(e.target.value)} />
                <button onClick={()=>pickCat("all")} className="btn"
