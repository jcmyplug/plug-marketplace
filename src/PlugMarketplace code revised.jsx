...
.plug {
  font-family: 'Inter', sans-serif; color: #16130F; background: #FBF8F2; min-height: 100vh;
  -webkit-font-smoothing: antialiased; -moz-osx-font-smoothing: grayscale;
  text-rendering: optimizeLegibility;
}
.plug button, .plug input, .plug select, .plug textarea { font-family: 'Inter', sans-serif; }
.plug h1, .plug h2, .plug h3 {
  font-family: 'Playfair Display', serif; letter-spacing: -0.02em;
  font-feature-settings: "liga" 1, "kern" 1;
}
.plug ::selection { background: rgba(255,92,40,0.18); color: #16130F; }
input { outline: none; }

/* ── Focus rings only on keyboard navigation (brand-warm) ── */
input:focus-visible { border-color: #FF5C28 !important; box-shadow: 0 0 0 3px rgba(255,92,40,0.16) !important; }
input:focus:not(:focus-visible) { border-color: #ECE6DA !important; box-shadow: none !important; }
textarea:focus-visible { outline: 2px solid rgba(255,92,40,0.30) !important; outline-offset: 2px; }
textarea:focus:not(:focus-visible) { outline: none !important; }
select:focus-visible { outline: 2px solid rgba(255,92,40,0.30); outline-offset: 1px; }
select:focus:not(:focus-visible) { outline: none; }

/* ── Buttons: refined easing for a smoother, more premium press ── */
.btn {
  transition: background 240ms cubic-bezier(0.4,0,0.2,1), color 240ms cubic-bezier(0.4,0,0.2,1),
              border-color 240ms cubic-bezier(0.4,0,0.2,1), transform 0.18s cubic-bezier(0.34,1.4,0.64,1),
              box-shadow 0.22s ease, opacity 0.15s ease;
  cursor: pointer; border: none;
}
.btn:hover { box-shadow: 0 6px 20px rgba(22,18,12,0.10); }
.btn:active { transform: scale(0.975); }
.btn:disabled { opacity: 0.42; cursor: not-allowed; transform: none; box-shadow: none; }

/* ── Cards: soft diffuse lift (premium elevation) ── */
.card {
  transition: box-shadow 0.3s cubic-bezier(0.16,1,0.3,1), transform 0.3s cubic-bezier(0.16,1,0.3,1);
}
.card:hover {
  transform: translateY(-2px);
  box-shadow: 0 18px 44px rgba(22,18,12,0.10), 0 4px 12px rgba(22,18,12,0.05);
}
.card:active {
  transform: translateY(0);
  box-shadow: 0 4px 12px rgba(22,18,12,0.07);
}

.pill { transition: background 240ms cubic-bezier(0.4,0,0.2,1), color 240ms cubic-bezier(0.4,0,0.2,1), border-color 240ms cubic-bezier(0.4,0,0.2,1); cursor: pointer; }

.subcard { transition: transform 0.24s cubic-bezier(0.16,1,0.3,1), box-shadow 0.24s cubic-bezier(0.16,1,0.3,1), border-color 0.24s ease; cursor: pointer; }
.subcard:hover { transform: translateY(-3px); box-shadow: 0 14px 36px rgba(22,18,12,0.10); border-color: rgba(184,151,90,0.4); }

.vendor-img-wrap { overflow: hidden; }
.vendor-img { transition: transform 0.6s cubic-bezier(0.16,1,0.3,1), opacity 0.3s ease; }
.vendor-img-wrap:hover .vendor-img { transform: scale(1.06); }
...
.skeleton {
  background: linear-gradient(120deg, #EFEADF 20%, #F7F3EB 28%, #EFEADF 43%);
...
.slide-right    { animation: slideRight 0.42s cubic-bezier(0.32, 0.72, 0, 1) both; }

/* ── Scrollbar (warm, refined) ── */
::-webkit-scrollbar { width: 6px; height: 6px; }
::-webkit-scrollbar-track { background: transparent; }
::-webkit-scrollbar-thumb { background: #D9D0C0; border-radius: 99px; }
::-webkit-scrollbar-thumb:hover { background: #C2B6A1; }

/* ── Stars ── */
.star-filled { color: #D99114; }
.star-empty  { color: #ECE6DA; }
...
.nav-glass {
  ...
  border-bottom: 1px solid rgba(236,230,218,0.7) !important;
}
...
.filter-pill {
  ...
  border: 1.5px solid #ECE6DA; background: #fff; cursor: pointer;
  white-space: nowrap; transition: all 0.18s ease;
}
.filter-pill.active {
  background: #16130F; color: #fff; border-color: #16130F;
}
.filter-pill:hover:not(.active) { border-color: #B8975A; color: #16130F; }

/* ── Vendor card V2 ── */
.vcard2 {
  border-radius: 20px; overflow: hidden; background: #fff;
  border: 1px solid #ECE6DA; transition: transform 0.32s cubic-bezier(0.16,1,0.3,1), box-shadow 0.32s cubic-bezier(0.16,1,0.3,1), border-color 0.32s ease;
  cursor: pointer;
}
.vcard2:hover {
  transform: translateY(-5px);
  box-shadow: 0 24px 54px rgba(22,18,12,0.13), 0 6px 16px rgba(22,18,12,0.06);
...
.heart-btn.active { background: #FCF1EE; }
...
.step-card:hover {
  ...
  box-shadow: 0 20px 44px rgba(22,18,12,0.10);
...
.rec-chip {
  ...
  background: #FFF2EB; border: 1.5px solid rgba(255,92,40,0.2); color: #FF5C28;
...
.avail-dot.open     { background: #0F9D74; box-shadow: 0 0 0 3px #ECFAF4; }
.avail-dot.busy     { background: #D99114; box-shadow: 0 0 0 3px #FBF6EA; }
.avail-dot.booked   { background: #E0533D; box-shadow: 0 0 0 3px #FCF1EE; }

/* ── Map placeholder ── */
.map-placeholder {
  background: linear-gradient(135deg, #F4EFE5 0%, #E8DCC2 100%);
  border-radius: 14px; display: flex; align-items: center;
  justify-content: center; font-size: 32px;
}

/* ── Smooth page transitions ── */
.page-enter { animation: fadeUp 0.36s cubic-bezier(0.16,1,0.3,1) both; }
...
const C = {
  /* ── Primary ── */
  orange: "#FF5C28", orangeHov: "#E64A18", orangeSoft: "#FFF2EB",
  orangeBorder: "rgba(255,92,40,0.20)", orangeGlow: "rgba(255,92,40,0.30)",
  ember: "#C2410C",

  /* ── Luxe metallic accent (champagne / gold) ── */
  gold: "#B8975A", goldSoft: "#FAF5EA", goldBorder: "rgba(184,151,90,0.30)",
  champagne: "#E8DCC2",

  /* ── Neutrals (warm) ── */
  black: "#16130F", darkGray: "#2A241D", midGray: "#6F665B",
  lightGray: "#A69C8E", border: "#ECE6DA", borderLight: "#F5F1E9",
  bg: "#FBF8F2", bgAlt: "#F4EFE5", white: "#fff",

  /* ── Semantic ── */
  green: "#0F9D74", greenSoft: "#ECFAF4", greenDark: "#065F46",
  red: "#E0533D",   redSoft: "#FCF1EE",
  amber: "#D99114", amberSoft: "#FBF6EA",
  blue: "#3B6FE0",  blueSoft: "#EEF3FC",
  ...
  gradPrimary: "linear-gradient(135deg, #FF5C28 0%, #FF8A3D 100%)",
  gradDark:    "linear-gradient(135deg, #16130F 0%, #2C2418 100%)",
  gradViolet:  "linear-gradient(135deg, #7A5CFF 0%, #5FD6FF 100%)",
  gradGold:    "linear-gradient(135deg, #B8975A 0%, #E2CD9C 100%)",
  gradHero:    "linear-gradient(102deg, rgba(15,12,9,0.86) 0%, rgba(15,12,9,0.55) 48%, rgba(15,12,9,0.14) 100%)",

  /* ── Elevation / shadows (soft, diffuse, warm-tinted) ── */
  shadowXs:     "0 1px 2px rgba(22,18,12,0.04)",
  shadowCard:   "0 1px 3px rgba(22,18,12,0.04), 0 6px 18px rgba(22,18,12,0.05)",
  shadowMd:     "0 4px 20px rgba(22,18,12,0.07), 0 2px 6px rgba(22,18,12,0.04)",
  shadowLg:     "0 18px 48px rgba(22,18,12,0.10), 0 6px 16px rgba(22,18,12,0.05)",
  shadowPopover:"0 10px 32px rgba(22,18,12,0.12), 0 2px 6px rgba(22,18,12,0.05)",
  shadowDrawer: "-8px 0 52px rgba(22,18,12,0.14)",
  shadowModal:  "0 40px 100px rgba(22,18,12,0.28), 0 12px 32px rgba(22,18,12,0.12)",
  shadowButton: "0 8px 24px rgba(255,92,40,0.30)",
  shadowViolet: "0 8px 24px rgba(122,92,255,0.30)",
};
...
const sb = (() => {
  ...
  return { rest, signUp, signIn, signOut, getUser, from, channel };
})();
...
function AdminPanel({ user, onClose }) {
  ...
  async function handleAction(vendorId, action) {
    setBusy(true);
    await setVendorStatus(vendorId, action === "approve" ? "approved" : "rejected",
    ...
    <div className="plug">
      ...
      {isHero && (
        <div style={{ position:"relative", height:480, overflow:"hidden" }}>
          ...
          <div style={{ position:"absolute", inset:0, background:C.gradHero }} />
          <div style={{ position:"absolute", inset:0, background:"linear-gradient(180deg, rgba(15,12,9,0.28) 0%, rgba(15,12,9,0) 30%, rgba(15,12,9,0) 70%, rgba(251,248,242,0.12) 100%)" }} />
          <div style={{ position:"absolute", inset:0, display:"flex", alignItems:"center", padding:"0 7% 0 6%" }}>
            <div style={{ maxWidth:560 }} className="fade-up">
              <div style={{ display:"inline-flex", alignItems:"center", gap:7, background:"rgba(255,255,255,0.10)", backdropFilter:"blur(12px)", WebkitBackdropFilter:"blur(12px)", border:"1px solid rgba(232,220,194,0.45)", borderRadius:99, padding:"6px 15px", marginBottom:18 }}>
                <Emoji e="⚡" size={11} />
                <span style={{ color:C.champagne, fontSize:11, fontWeight:800, letterSpacing:"0.12em" }}>HOUSTON'S EVENT MARKETPLACE</span>
              </div>
              <h1 style={{ fontFamily:"'Playfair Display', serif", fontSize:"clamp(34px,4.5vw,58px)", fontWeight:900, color:"#fff", lineHeight:1.08, letterSpacing:"-0.04em", margin:"0 0 16px" }}>
                ...
                <span className="gradient-text">Build your entire event.</span>
              </h1>
        ...
        <div style={{ maxWidth:1280, margin:"0 auto", padding:`${isHero?"28px":"32px"} 24px 100px` }}>
          ...
          {!showSubGrid && activeCat !== "build" && (
            <div className="fade-up">
              ...
              <RecommendationStrip recs={recs} onAdd={addToCart} onView={(vv)=>setVendorPage(vv)} cart={cart} />
