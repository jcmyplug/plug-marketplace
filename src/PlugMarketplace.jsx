...
/* ─── DESIGN TOKENS ─────────────────────────────────────────────────────────── */
const BUILD_VERSION = "2026-08-06-profile-services-v3";
const C = {
  /* ── Primary ── */
...
async function fetchOSMSuggestions(query) {
  ...
  try {
    ...
    return (Array.isArray(data) ? data : []).map(parseGeoResult).filter(x => x.full).map(x => ({ ...x, kind: "osm" }));
  } catch { return []; }
}
...
function AddressAutocomplete({ onSelect, placeholder }) {
  ...
  function runGoogle(v) {
    acService.current.getPlacePredictions(
      ...
      (preds, status) => {
        if (status !== "OK" || !preds) { setResults([]); setLoading(false); return; }
  ...
  function onType(v) {
    ...
    timer.current = setTimeout(async () => {
      ...
      else { const r = await fetchOSMSuggestions(v); setResults(r); setLoading(false); }
    }, 350);
  }

  function pick(r) {
    setOpen(false); setResults([]); setQ(r.full);
    ...
    <div className="plug">
      ...
      <div style={{ background:C.black, padding:"28px 6%", display:"flex",
        ...
        <div style={{ display:"flex", alignItems:"center", gap:7 }}>
          ...
          <span style={{ fontSize:11, color:"#555", marginLeft:6 }}>Houston, TX · © 2026 · <span title="build version">{BUILD_VERSION}</span></span>
        </div>
