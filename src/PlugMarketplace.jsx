...
/* ─── ADDRESS AUTOCOMPLETE ───────────────────────────────────────────────────
   Real address search so customers pick a precise, findable place rather than
   typing a vague one. Uses a maps geocoder; on pick, fills street/city/state/zip.
   Works with no setup. To use Google Places instead, set GOOGLE_MAPS_KEY and the
   component will prefer it (see fetchAddressSuggestions). */
const GOOGLE_MAPS_KEY = ""; // optional — paste a Google Maps API key to use Google

/* Parse a geocoder result into the app's structured address fields. */
function parseGeoResult(r) {
  ...
  return { street, city, state, zip, full: r.display_name || street || "" };
}
...
async function fetchAddressSuggestions(query) {
  ...
  try {
    ...
    return (Array.isArray(data) ? data : []).map(parseGeoResult).filter(x => x.full);
  } catch { return []; }
}
...
function AddressAutocomplete({ onSelect, placeholder }) {
  ...
  function onType(v) {
    ...
    timer.current = setTimeout(async () => {
      const r = await fetchAddressSuggestions(v);
      setResults(r); setLoading(false);
    }, 400);   // debounce — respects geocoder rate limits
  }

  function pick(r) {
    onSelect(r);
    setQ(r.full); setOpen(false); setResults([]);
  }
      ...
      {open && (q.trim().length >= 3) && (
        <div style={{ position:"absolute", top:46, left:0, right:0, zIndex:30, background:"#fff",
          ...
          {loading ? (
            ...
            <p style={{ margin:0, padding:"10px 12px", fontSize:12, color:C.midGray }}>
...
function CartPanel({ cart, onRemove, onClose, onSubmitRequests, user, setAuthModal, budget, onSetBudget, initialDetails, onDetailsChange }) {
  ...
  /* Save everything the customer enters here into the shared store, so it's
     prefilled next time and survives a page refresh — no re-typing. */
  useEffect(() => {
    if (!onDetailsChange) return;
    onDetailsChange({
      eventType, eventDate, guests: eventGuests, venue: eventVenue, message,
      venueType, street, addr2, city, state: stateAbbr, zip, startTime, endTime, access,
    });
  }, [eventType, eventDate, eventGuests, eventVenue, message, venueType, street, addr2, city, stateAbbr, zip, startTime, endTime, access]);
              ...
              <div style={{ background:"#FFF7ED", borderRadius:14, padding:"14px 16px",
                ...
                <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
                  ...
                  <div>
                    ...
                    <ClickCalendar value={eventDate} onChange={(d) => {
                      setEventDate(d);
                  ...
                  <input placeholder="Venue name (optional) — e.g. The Grand Ballroom" value={eventVenue}
                    ...
                    style={{ height:40, padding:"0 12px", border:`1px solid ${C.border}`,
                             borderRadius:9, fontSize:13, background:"#fff" }} />
                  {/* Map-backed search — pick a precise address so vendors can find it. */}
                  <AddressAutocomplete
                    placeholder="🔍 Search the event address (recommended)…"
                    onSelect={(a) => {
                      if (a.street) setStreet(a.street);
                      if (a.city)   setCity(a.city);
                      if (a.state)  setStateAbbr(a.state.length > 3 ? a.state.slice(0,2).toUpperCase() : a.state.toUpperCase());
                  ...
                  <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
                    <div>
                      ...
                      <TimeGrid value={startTime} onChange={(t) => {
                        setStartTime(t);
                    ...
                    <div>
                      ...
                      <TimeGrid value={endTime} onChange={setEndTime} after={startTime} />
                    </div>
                  </div>
                  <div>
                  ...
                  return (
                    <button key={s.id} type="button" onClick={() => setSelId(s.id)} className="btn"
                      ...
                      <div style={{ flex:1, minWidth:0 }}>
                        ...
                        <p style={{ margin:"2px 0 0", fontSize:11, color:C.midGray }}>
                          {catLabelOf(s.category)}{s.category !== "places" ? (s.offsite ? " · 🚗 available off-site" : " · 📍 on-site only") : ""}{s.description ? " · " + s.description.slice(0, 60) + (s.description.length > 60 ? "…" : "") : ""}
                        </p>
                      </div>
...
function ServicesManager({ vendorId }) {
  ...
  const blank = { category:"food", subcategory:"", name:"", service_type:"", description:"",
                  price_value:"", capacity:"", photos:[], packages:[], active:true, offsite:false };
      ...
      {editing && (
        <div style={{ marginTop:12, background:"#fff", border:`1.5px solid ${C.orangeBorder}`,
          ...
          <input style={F} value={editing.name || ""}
            ...
            placeholder="e.g. El Fuego Taco Truck, Sweet Wheels Dessert Truck" />
          <p style={{ margin:"3px 0 0", fontSize:10.5, color:C.lightGray }}>
            Give each service its own name so customers can tell your offerings apart.
          </p>
