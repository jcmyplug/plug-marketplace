...
async function submitReviewDB({ bookingId, authorId, subjectId, direction, rating, body }) {
...
function serviceLabel(svc) {
  return svc?.service_type || subLabelOf(svc) || catLabelOf(svc?.category) || "Service";
}
...
function ClickCalendar({ value, onChange }) {
        ...
        {cells.map((d, i) => {
          ...
          return (
            <button key={iso} type="button" disabled={past}
              onClick={() => onChange(iso)} className="btn"
              style={{ aspectRatio:"1", border:"none", borderRadius:9, fontSize:13, fontWeight: sel?800:600,
                       ...
                       color: sel ? "#fff" : past ? "#D1D5DB" : C.black }}>
              {d.getDate()}
            </button>
          );
        })}
...
function TimeGrid({ value, onChange, after }) {
  ...
  return (
    <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill, minmax(84px, 1fr))", gap:6,
      ...
      {opts.map(t => {
        const sel = value === t;
        return (
          <button key={t} type="button" onClick={() => onChange(t)} className="btn"
            style={{ padding:"8px 4px", borderRadius:9, fontSize:12.5, fontWeight: sel?800:600, cursor:"pointer",
                     border:`1.5px solid ${sel ? C.orange : C.border}`,
                     background: sel ? "#FFF7ED" : "#fff", color: sel ? C.orange : C.black }}>
            {fmtTime12(t)}
          </button>
        );
      })}
              ...
              <div className="vendor-grid" style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill, minmax(260px, 1fr))", gap:16 }}>
                {vendors.map(v => (
                  <VCard key={v.id} v={v} inCart={inCart(v.id)}
                    isFav={favorites.includes(v.vendorId)} onAdd={addWithDetails} onRemove={rmFromCart}
...
function CartPanel({ cart, onRemove, onClose, onSubmitRequests, user, setAuthModal, budget, onSetBudget, initialDetails }) {
  ...
  const [stateAbbr,  setStateAbbr]  = useState(initialDetails?.city ? "TX" : "");
...
function VendorProfile({ vendor, user, reviews, onBack, onAddReview, onVendorReply, onAddToCart, inCart }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (cancel) return;
      console.log(`[PLUG] VendorProfile load: id=${vendorId} profile=${prof ? "OK" : "none"} services=${(svcs||[]).length}`);
      setFull(prof);
  ...
  const disp = {
    ...
    travelMiles: full?.travel_miles ?? vendor.travelMiles ?? 0,
    yearsInBiz:  full?.years_in_biz ?? vendor.yearsInBiz ?? 0,
    serviceAreas:full?.service_areas || vendor.serviceAreas || "",
  ...
  /* Gallery: selected offering's photos first, then business photos, then any
     other service photos, then the card image — deduped. */
  const galleryPhotos = (() => {
    const merged = [
      ...(selService ? parsePhotos(selService.photos) : []),
      ...parsePhotos(full?.photos),
      ...services.flatMap(s => parsePhotos(s.photos)),
      ...(vendor.photos || []),
      vendor.img,
    ].filter(Boolean);
  ...
  function offeringForCart() {
    const label = selService ? serviceLabel(selService) : (vendor.serviceName || vendor.type);
    return {
      ...vendor,
      name:        disp.name,
      serviceId:   selService ? selService.id : vendor.serviceId,
      serviceName: label,
      cat:         disp.cat,
  ...
  const vRevs = isLiveVendor
    ? dbRevs.map(r => ({ id: r.id, uid: r.authorId, uname: r.authorName || "Verified customer",
                          rating: r.rating, text: r.body, date: new Date(r.createdAt).toLocaleDateString(), reply: r.reply }))
    : seedRevs;
  ...
  const isVendorOwner = user?.type === "vendor" && user?.vendorId === vendorId;
  ...
  async function submitReview() {
    ...
    if (isLiveVendor) {
      const res = await submitReviewDB({
        ...
        rating: newRating, body: newText.trim(),
      });
    ...
    <div className="fade-up" style={{ maxWidth:1040, margin:"0 auto" }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 300px", gap:24, alignItems:"start" }}>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:18 }}>
          ...
          {/* Offerings — every service this vendor provides, selectable */}
          {services.length > 0 && (
            <div style={{ background:"#fff", border:`1px solid ${C.border}`, borderRadius:16, padding:"20px 22px", marginTop:20 }}>
              <h2 style={{ fontSize:17, fontWeight:800, margin:"0 0 4px" }}>Services & offerings</h2>
              <p style={{ fontSize:12, color:C.midGray, margin:"0 0 14px" }}>
                {services.length === 1 ? "What this vendor provides." : `Pick from ${services.length} offerings — the booking card updates to match.`}
              </p>
              <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
                {services.map(s => {
                  const on = s.id === selId;
                  ...
                  return (
                    <button key={s.id} type="button" onClick={() => setSelId(s.id)} className="btn"
                      style={{ display:"flex", gap:12, alignItems:"center", textAlign:"left", cursor:"pointer",
                               padding:"10px 12px", borderRadius:12,
                               border:`1.5px solid ${on ? C.orange : C.border}`,
                               background: on ? "#FFF7ED" : "#fff" }}>
                      {pics[0]
                        ? <img src={pics[0]} alt="" style={{ width:52, height:52, borderRadius:9, objectFit:"cover", flexShrink:0 }} />
                        : <div style={{ width:52, height:52, borderRadius:9, background:"#F3F4F6", flexShrink:0,
                                        display:"flex", alignItems:"center", justifyContent:"center", fontSize:22 }}>🏪</div>}
                      <div style={{ flex:1, minWidth:0 }}>
                        <p style={{ margin:0, fontSize:13.5, fontWeight:800, color: on ? C.orange : C.black }}>
                          {on ? "● " : "○ "}{serviceLabel(s)}
                        </p>
                        <p style={{ margin:"2px 0 0", fontSize:11, color:C.midGray }}>
                          {catLabelOf(s.category)}{s.description ? " · " + s.description.slice(0, 60) + (s.description.length > 60 ? "…" : "") : ""}
                        </p>
                      </div>
                      <span style={{ fontSize:12.5, fontWeight:800, color:C.black, whiteSpace:"nowrap" }}>{price}</span>
                    </button>
                  );
                })}
              </div>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:10 }}>
            <Stat icon="⭐" label="Rating"    value={`${avgRating} / 5`} sub={`${disp.revCount}+ reviews`} accent="#F59E0B" />
        ...
        <div style={{ position:"sticky", top:80 }}>
          <div style={{ background:"#fff", border:`1px solid ${C.border}`, borderRadius:18, padding:"20px 20px 22px",
            ...
            <button onClick={() => !inCart && onAddToCart(offeringForCart())} className="btn"
              ...
              {inCart ? "✓ Added to cart" : disp.instant ? "⚡ Book now" : "Send booking request"}
            </button>
            ...
            <div style={{ background:"#fff", border:`1px solid ${C.border}`, borderRadius:14, padding:"16px 18px" }}>
              ...
              {requests.length === 0 ? (
                ...
                <div key={r.id} style={{ borderTop:`1px solid ${C.border}`, padding:"12px 0" }}>
                  ...
                  {/* After a confirmed booking, the vendor can rate the customer.
                     Symmetric with the customer reviewing the vendor. */}
...
export default function PlugApp() {
  ...
  const [buildDetails, setBuildDetails] = useState(null);   // details prefilled from Build My Event
