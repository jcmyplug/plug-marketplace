...
async function getApprovedVendors() {
  ...
  /* Preferred path: one card per SERVICE (a vendor can offer many).
     Falls back to the legacy one-card-per-vendor shape when
     vendor-services-setup.sql hasn't been run yet. */
  ...
  if (error) console.warn("[PLUG] vendor_services unavailable — using legacy listings. Run vendor-services-setup.sql.", error);
  const { data: legacy } = await sb.from("vendor_profiles")
    .select("*")
    .eq("verification_status", "approved")
    .order("created_at", { ascending: false })
    .get();
  return (legacy || []).map(dbVendorToCard);
}
...
function dbServiceToCard(s, v) {
  ...
  const label = s.service_type || subLabel || catLabel || "Service";
  return {
    id:          "svc_" + s.id,
    dbId:        v.id,
    ...
    type:        subLabel && s.service_type ? `${label} · ${subLabel}` : label,
    city:        [v.biz_city, v.biz_state].filter(Boolean).join(", ") || "Houston, TX",
    rating:      0,
    revCount:    0,
...
function AccountPanel({ user, justSent, onClose, onLogout, onListingSaved }) {
            ...
            <>
              ...
              <div style={{ background:"#FFF7ED", borderRadius:14, padding:"14px 16px",
                ...
                <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
                  ...
                  <div>
                    <label style={{ fontSize:10, fontWeight:700, color:C.midGray, display:"block", marginBottom:3 }}>
                    ...
                    <input type="date" value={eventDate}
                      ...
                      style={{ width:"100%", height:40, padding:"0 12px",
                               border:`1px solid ${eventDate ? C.border : "#FCA5A5"}`,
...
function VendorProfile({ vendor, user, reviews, onBack, onAddReview, onVendorReply, onAddToCart, inCart }) {
  ...
  /* Pricing options the vendor has actually priced (unpriced ones stay hidden).
     Defaults to the cheapest so there's always a valid selection. */
  const vendorPkgs = visiblePackages(vendor.packages);
  const [pickedPkg, setPickedPkg] = useState(
    vendor.selectedPackage || (vendorPkgs.length ? vendorPkgs.reduce((a,b)=> b.price<a.price?b:a) : null)
  );
        ...
        <div style={{ position:"sticky", top:80 }}>
          <div style={{ background:"#fff", border:`1px solid ${C.border}`, borderRadius:18, padding:"20px 20px 22px",
            ...
            <p style={{ fontFamily:"'Playfair Display',serif", fontSize:28, fontWeight:800, color:C.black, margin:"0 0 4px" }}>
              {pickedPkg ? "$" + pickedPkg.price.toLocaleString() : (vendor.price || "Contact for pricing")}
            </p>
            ...
            <button onClick={() => !inCart && onAddToCart(
                pickedPkg
                  ? { ...vendor, selectedPackage: pickedPkg, pv: pickedPkg.price,
                      price: "$" + pickedPkg.price.toLocaleString() }
                  : vendor
              )} className="btn"
...
function ServicesManager({ vendorId }) {
  ...
  const blank = { category:"food", subcategory:"", service_type:"", description:"",
                  price_value:"", capacity:"", photos:[], packages:[], active:true };
  ...
  return (
    <div style={{ marginTop:18, borderTop:`1px solid ${C.border}`, paddingTop:16 }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
        <div>
          <p style={{ margin:0, fontSize:13, fontWeight:800, color:C.black }}>Your services</p>
              ...
              <div key={s.id} style={{ display:"flex", gap:10, alignItems:"center", background:"#F9FAFB",
                ...
                <div style={{ flex:1, minWidth:0 }}>
                  <p style={{ margin:0, fontSize:13, fontWeight:700, color:C.black }}>
                    {s.service_type || s.subcategory || catLabel(s.category)}
                  </p>
                  <p style={{ margin:"1px 0 0", fontSize:11, color:C.midGray }}>
                    {catLabel(s.category)}
      ...
      {editing && (
        <div style={{ marginTop:12, background:"#fff", border:`1.5px solid ${C.orangeBorder}`,
          ...
          <select style={F} value={editing.category} onChange={e => setField("category", e.target.value)}>
          ...
          <label style={L}>Service name <span style={{fontWeight:400}}>(e.g. “Taco Truck”, “Wedding DJ”)</span></label>
          <input style={F} value={editing.service_type} onChange={e => setField("service_type", e.target.value)} />
          ...
          <div style={{ display:"flex", gap:8 }}>
            ...
            <div style={{ flex:1 }}>
              <label style={L}>Capacity for this service</label>
              <input style={F} value={editing.capacity} onChange={e => setField("capacity", e.target.value)}
          ...
          <label style={{ display:"flex", alignItems:"center", gap:8, marginTop:10, cursor:"pointer" }}>
            ...
            <span style={{ fontSize:12, color:C.midGray }}>Visible to customers</span>
          </label>
      ...
      <div onClick={e=>e.stopPropagation()}
        ...
        {loading || !f ? (
          ...
          <>
            ...
            <div style={{ background:"#F9FAFB", border:`1px solid ${C.border}`, borderRadius:10,
              ...
              <div style={{ display:"flex", gap:8 }}>
                ...
                <div style={{ flex:1 }}>
                  ...
                  {f.category === "places" ? (
                    <div style={{ ...F, display:"flex", alignItems:"center", color:C.midGray,
                                  fontSize:12, background:"#F9FAFB", borderStyle:"dashed" }}>
                      📍 Fixed location
                    </div>
                  ) : (
            ...
            {/* Multiple services — one vendor can sell across categories */}
