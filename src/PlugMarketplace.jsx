...
async function getApprovedVendors() {
  ...
  const { data } = await sb.from("vendor_profiles")
    ...
    .get();
  return (data || []).map(dbVendorToCard);
}
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
        ...
        <label style={{ display:"flex", alignItems:"flex-start", gap:10, cursor:"pointer" }}>
          ...
          <span style={{ fontSize:12, color:C.midGray, lineHeight:1.65 }}>
            ...
            <span onClick={()=>setLegalView("Terms")}
              style={{ color:C.orange, fontWeight:600, cursor:"pointer", textDecoration:"underline" }}>Terms of Service</span>,{" "}
            <span onClick={()=>setLegalView("Privacy")}
              style={{ color:C.orange, fontWeight:600, cursor:"pointer", textDecoration:"underline" }}>Privacy Policy</span>, and{" "}
            <span onClick={()=>setLegalView("Marketplace rules")}
              style={{ color:C.orange, fontWeight:600, cursor:"pointer", textDecoration:"underline" }}>Marketplace Rules</span>.
            {role === "vendor" && " I understand my business must be verified before listing services."}
            <br />
            <span style={{ fontSize:11, color:"#B45309" }}>
              I understand that if I do not follow these terms, PLUG may suspend, block, or permanently
              remove my account and listings.
            </span>
          </span>
        </label>
        {role === "vendor" && (
...
function AccountPanel({ user, onClose, onLogout, onListingSaved }) {
...
function VendorListingEditor({ user, onClose, onSaved }) {
  ...
  async function save() {
    ...
    const res = await saveMyListing(user.id, {
      ...
      travel_miles:  f.travel_miles === "" ? null : parseInt(f.travel_miles) || null,
  ...
  return (
    <div className="modal-overlay" onClick={onClose}
      ...
      <div onClick={e=>e.stopPropagation()}
        ...
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
          <h2 style={{ margin:0, fontSize:17, fontWeight:800 }}>🏪 Edit my listing</h2>
          <button onClick={onClose} className="btn"
        ...
        {loading || !f ? (
          ...
          <>
            ...
            <select style={F} value={f.category} onChange={e=>set("category", e.target.value)}>
              {CATEGORIES.filter(c=>c.id!=="all"&&c.id!=="build").map(c=>(
                <option key={c.id} value={c.id}>{c.label || c.name || c.id}</option>
              ))}
            </select>

            <label style={L}>Service type <span style={{fontWeight:400}}>(e.g. “Taco Truck”, “Wedding DJ”)</span></label>
            <input style={F} value={f.service_type} onChange={e=>set("service_type", e.target.value)} />

            <label style={L}>Describe the service you provide *</label>
            <textarea value={f.description} onChange={e=>set("description", e.target.value)}
              rows={4} placeholder="Tell customers what you offer, what's included, and what makes you different."
              style={{ ...F, height:"auto", padding:"10px 12px", resize:"vertical", fontFamily:"inherit" }} />

            <label style={L}>Starting price <span style={{fontWeight:400}}>(leave blank for “Contact for pricing”)</span></label>
            <input style={F} type="number" min="0" value={f.price_value}
              onChange={e=>set("price_value", e.target.value)} placeholder="e.g. 1200" />

            {/* Autopopulated from signup — editable */}
            <div style={{ background:"#F9FAFB", border:`1px solid ${C.border}`, borderRadius:10,
              ...
              <p style={{ margin:0, fontSize:11, fontWeight:700, color:C.midGray }}>
                From your application — edit any time
              </p>
              <label style={L}>Capacity</label>
              <input style={F} value={f.capacity} onChange={e=>set("capacity", e.target.value)} placeholder="e.g. 50–300 guests" />
              <label style={L}>Project size</label>
              <input style={F} value={f.project_size} onChange={e=>set("project_size", e.target.value)} placeholder="e.g. Small to Large" />
              <div style={{ display:"flex", gap:8 }}>
                <div style={{ flex:1 }}>
                  <label style={L}>Years in business</label>
                  <input style={F} type="number" min="0" value={f.years_in_biz} onChange={e=>set("years_in_biz", e.target.value)} />
                </div>
                <div style={{ flex:1 }}>
                  <label style={L}>Travel radius (miles)</label>
                  <input style={F} type="number" min="0" value={f.travel_miles} onChange={e=>set("travel_miles", e.target.value)} />
                </div>
              </div>
              <label style={L}>Service areas</label>
              <input style={F} value={f.service_areas} onChange={e=>set("service_areas", e.target.value)} />
              <label style={L}>Availability</label>
              <input style={F} value={f.schedule} onChange={e=>set("schedule", e.target.value)} />
            </div>
    ...
    <div className="modal-overlay" onClick={onClose}
      ...
      <div onClick={e=>e.stopPropagation()}
        style={{ background:"#fff", borderRadius:16, width:"100%", maxWidth:560,
                 maxHeight:"85vh", overflowY:"auto", padding:"24px 24px 28px" }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
          <h2 style={{ margin:0, fontSize:19, fontWeight:800 }}>{page}</h2>
          <button onClick={onClose} className="btn"
            style={{ border:"none", background:"#F3F4F6", borderRadius:99, width:30, height:30 }}>✕</button>
        </div>
        <p style={{ fontSize:11, color:"#888", margin:"6px 0 14px" }}>Last updated {LEGAL_UPDATED}</p>
        {sections.map(([h, body]) => (
          <div key={h} style={{ marginBottom:16 }}>
            <h3 style={{ margin:"0 0 5px", fontSize:14, fontWeight:800 }}>{h}</h3>
            <p style={{ margin:0, fontSize:13, lineHeight:1.65, color:"#444" }}>{body}</p>
          </div>
        ))}
      </div>
    </div>
...
function VendorDashboard({ user, onLogout }) {
  ...
  async function respond(reqId, status) {
    ...
    setBusyId(null);
    if (!updated) {
      setErr("Could not update that request. If this keeps happening, your account may not have permission yet (database policy).");
      return;
    }
    reload();
  }
  ...
  const TABS = [["overview","Overview"],["requests","Requests"],["listing","My listing"],
        ...
        {!isApproved && (
          <div style={{ background:"#FFFBEB", border:"1px solid #FCD34D", borderRadius:12,
            ...
            <p style={{ margin:"4px 0 0", fontSize:12, color:"#B45309" }}>
              You can build your listing now. It becomes visible to customers once approved.
            </p>
          </div>
        )}
        {isApproved && !listingComplete && (
          <div style={{ background:"#EFF6FF", border:"1px solid #BFDBFE", borderRadius:12,
            ...
            <p style={{ margin:0, fontSize:13, fontWeight:800, color:"#1E40AF" }}>
              Finish your listing to get booked
            </p>
          ...
          {tab === "overview" && (
            <>
              <div style={{ display:"flex", gap:10, flexWrap:"wrap", marginBottom:16 }}>
                <Metric label="NEW REQUESTS" value={pending.length} sub="awaiting your reply" accent={C.orange} />
                <Metric label="CONFIRMED"    value={confirmed.length} sub="booked jobs" accent={C.green} />
                <Metric label="DECLINED"     value={declined.length} sub="not taken" />
                <Metric label="RATING"       value={listing?.rating ? Number(listing.rating).toFixed(1) : "—"} sub="from reviews" accent="#F59E0B" />
                <Metric label="PHOTOS"       value={(listing?.photos||[]).length} sub="on your listing" />
              </div>
                ...
                <div key={r.id} style={{ borderTop:`1px solid ${C.border}`, padding:"12px 0" }}>
                  <div style={{ display:"flex", justifyContent:"space-between", gap:10, flexWrap:"wrap" }}>
                    <div style={{ flex:1, minWidth:200 }}>
                      ...
                      <p style={{ margin:"3px 0 0", fontSize:12, color:C.midGray }}>
                        📅 {r.eventDate || "Date TBD"} · 📍 {r.venue || "Venue TBD"}
                      </p>
                      {r.message && (
                        <p style={{ margin:"5px 0 0", fontSize:12, color:"#444", fontStyle:"italic" }}>
                          “{r.message}”
                        </p>
                      )}
                      <p style={{ margin:"4px 0 0", fontSize:10, color:C.lightGray, fontFamily:"monospace" }}>
                        {r.id}
                      </p>
                    </div>
                    <span style={{ alignSelf:"flex-start", padding:"3px 10px", borderRadius:99,
                                   fontSize:10, fontWeight:800,
                                   background: r.status==="confirmed" ? C.greenSoft : r.status==="declined" ? "#FEF2F2" : "#FFFBEB",
                                   color: r.status==="confirmed" ? C.green : r.status==="declined" ? "#EF4444" : "#D97706" }}>
                      {r.status === "confirmed" ? "✓ Confirmed" : r.status === "declined" ? "✗ Declined"
          ...
          {tab === "listing" && (
            <div style={{ background:"#fff", border:`1px solid ${C.border}`, borderRadius:14, padding:"16px 18px" }}>
              ...
              {[["Business", listing?.business_name],
                ...
                ["Project size", listing?.project_size],
...
export default function PlugApp() {
  ...
  function matchesWhere(v, where) {
    ...
    return `${v.city||""} ${v.serviceAreas||""} ${v.bizCity||""} ${v.bizState||""}`
      .toLowerCase().includes(w);
  }

  /* Can this vendor handle the requested headcount?
     capacity is free text like "50–300 guests" — we read the largest number
     as the ceiling. If we can't parse it, we don't exclude the vendor. */
  ...
  }, [activeCat, activeSub, search, sortBy, filters, activePackage, dbVendors, qWhere, qGuests]);
    ...
    <div className="plug">
      ...
      {activeCat !== "build" && (
        <div style={{ position:"sticky", top:64, zIndex:191,
          ...
          {(qWhere || qWhen || qGuests) && (
            <button onClick={()=>{setQWhere("");setQWhen("");setQGuests("");}} className="btn"
            ...
            <div style={{ maxWidth:560 }} className="fade-up">
              ...
              {(qWhere || qGuests || qWhen) && (
                <p style={{ fontSize:12, color:"rgba(255,255,255,0.85)", marginTop:8, fontWeight:600 }}>
                  ...
                  <button onClick={()=>{setQWhere("");setQWhen("");setQGuests("");}}
