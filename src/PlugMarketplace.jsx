...
function AdminAccounts() {
  ...
  async function sendMsg() {
    ...
    const res = await adminSendMessage(msgFor.id, msgKind, msgSubject.trim(), msgBody.trim());
    setBusyId(null);
    if (!res.ok) { setErr(res.error); return; }
    setMsgFor(null); setMsgSubject(""); setMsgBody(""); setMsgKind("message");
    flash(msgKind === "warning" ? "Warning sent." : "Message sent.");
  }
...
function InfoPageModal({ page, onClose }) {
  ...
  /* Escape closes it — and on mobile there's no Escape, so the header ✕ and the
     footer Close button both stay fixed in view while the text scrolls. */
  useEffect(() => {
    ...
    <div className="modal-overlay" onClick={onClose}
      ...
      <div onClick={e=>e.stopPropagation()}
        ...
        <div style={{ padding:"18px 22px 12px", borderBottom:`1px solid ${C.border}`,
          ...
          <p style={{ fontSize:11, color:"#888", margin:"6px 0 0" }}>Last updated {LEGAL_UPDATED}</p>
        </div>

        {/* Scrollable body */}
        <div style={{ overflowY:"auto", padding:"16px 22px", flex:1, WebkitOverflowScrolling:"touch" }}>
          {sections.map(([h, body]) => (
        ...
        {/* Fixed footer — always reachable, however long the document is */}
