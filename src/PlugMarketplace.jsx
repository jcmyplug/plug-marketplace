...
async function sendInquiry({ userId, vendorId, serviceId, serviceName, body }) {
  if (IS_PREVIEW) return { ok: true };
  ...
  if (error) return { ok: false, error: error.message || "Could not send your inquiry." };
  return { ok: true };
}
        ...
        <div style={{ gridColumn:"1 / 3", position:"relative", cursor:"pointer" }}
          ...
          <div style={{ position:"absolute", bottom:18, left:20, right:20 }}>
            ...
            <p style={{ color:"rgba(255,255,255,0.85)", fontSize:13, margin:0 }}>
              by <span style={{ fontWeight:800, color:"#fff" }}>{disp.name}</span>{disp.city ? " · " + disp.city : ""}
            </p>
          </div>
        </div>
        <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
...
function ServicesManager({ vendorId }) {
  ...
  const blank = { category:"food", subcategory:"", name:"", service_type:"", description:"",
                  price_value:"", capacity:"", photos:[], packages:[], active:true, offsite:false, travel_miles:"", service_areas:"", addons:[] };
...
function VendorDashboard({ user, onLogout }) {
  ...
  const TABS = [["overview","Overview"],["requests","Requests"],["inquiries","Inquiries"],["listing","My listing"],
              ...
              {reqView === "calendar" ? (
                ...
                <div key={r.id} style={{ borderTop:`1px solid ${C.border}`, padding:"14px 0" }}>
                  ...
                  {isConfirmedStatus(r.status) && (
                    <div style={{ display:"flex", gap:8, marginTop:12 }}>
                      <button onClick={()=>{ if (window.confirm("Cancel this confirmed booking? The customer will be notified.")) respond(r.id,"cancelled"); }}
...
export default function PlugApp() {
  ...
  /* After a refresh, re-open whichever listing the person was viewing — resolved
     from freshly loaded cards so the data is never stale. */
  useEffect(() => {
    if (vendorPage || !openCardId || !dbVendors.length) return;
