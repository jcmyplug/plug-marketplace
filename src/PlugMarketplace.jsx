...
async function fetchPhotonSuggestions(query) {
  const url = "https://photon.komoot.io/api/?limit=6&lang=en&q=" + encodeURIComponent(query);
  try {
    ...
    return (data.features || []).map(f => {
      ...
      return { street, city, state: p.state || "", zip: p.postcode || "", full, kind: "photon" };
    }).filter(x => x.full);
  } catch { return []; }
}
...
function AddressAutocomplete({ onSelect, placeholder }) {
  ...
  function onType(v) {
    ...
    timer.current = setTimeout(async () => {
      ...
      else { const r = await fetchOSMSuggestions(v); setResults(r); setLoading(false); setShowNoMatch(r.length === 0); }
    }, 350);
  }
        ...
        <div style={{ position:"absolute", top:46, left:0, right:0, zIndex:30, background:"#fff",
          ...
          {loading ? (
            ...
            <div style={{ padding:"10px 12px", display:"flex", alignItems:"center", gap:8 }}>
              <p style={{ margin:0, fontSize:12, color:C.midGray, flex:1 }}>
