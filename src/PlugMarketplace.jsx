...
const SECURITY_HEADERS = Object.freeze({
  ...
  meta: {
    csp: [
      ...
      "img-src 'self' https://images.unsplash.com https://cdn.jsdelivr.net https://*.supabase.co data: blob:",
...
function AuthModal({ onClose, onAuth }) {
  ...
  function validateStep() {
    ...
    if (step === 3 && role === "vendor") {
      ...
      if (!form.travelMiles){ setErr("Please choose your travel radius."); return false; }
      if (!form.projectSize){ setErr("Please choose your typical project size."); return false; }
      return true;
    }
  ...
  const renderStep = () => {
    ...
    if (step === 3 && role === "vendor") return (
      <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
        ...
        {/* Capacity, travel radius, project size — dropdowns to avoid typos */}
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
          <div>
            {sel("capacity", [
              ["0-20 guests","0–20 guests"],
              ["20-50 guests","20–50 guests"],
              ["50-100 guests","50–100 guests"],
              ["100-250 guests","100–250 guests"],
              ["250-500 guests","250–500 guests"],
              ["500+ guests","500+ guests"],
            ], "Guest capacity *")}
          ...
          <div>
            ...
            <p style={{ margin:"2px 0 0", fontSize:9, color:C.lightGray }}>Max distance from your base</p>
          </div>
        </div>
        {sel("projectSize", [
          ["Intimate","Intimate (small gatherings)"],
          ["Medium","Medium events"],
          ["Large","Large events"],
          ["Any size","Any size — small to large"],
...
function AccountPanel({ user, onClose, onLogout, onListingSaved }) {
  ...
  async function modifyRequest(reqId, changes) {
    ...
    const row = (Array.isArray(data) ? data[0] : null) || {};
    setRequests(r => r.map(req => req.id === reqId ? { ...req, ...changes } : req));
    setActionMsg("Booking updated. We've emailed the vendor the new details.");
    notifyBookingChange({
      requestId:  reqId,
      status:     "modified",
      customerId: user.id,
...
function CartPanel({ cart, onRemove, onClose, onSubmitRequests, user, setAuthModal, budget, onSetBudget }) {
  ...
  /* Which cart vendors are unavailable on the chosen date */
  function vendorsUnavailableOn(dateStr) {
    if (!dateStr) return [];
    return cart.filter(v => {
      const id = v.vendorId || v.dbId || String(v.id).replace(/^db_/, "");
      const a = vendorAvail[id];
      if (!a) return false;
      return (a.blocked || []).includes(dateStr) || (a.confirmed || []).includes(dateStr);
    });
  }
  ...
  async function handleSubmit() {
    ...
    const busy = vendorsUnavailableOn(eventDate);
    if (busy.length) {
      setErr(`${busy.map(v=>v.name).join(", ")} ${busy.length===1?"is":"are"} not available on ${eventDate}. Choose another date or remove them.`);
      return;
    }
    setSubmitting(true);
                ...
                <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
                  ...
                  <input type="date" value={eventDate}
                    ...
                    onChange={e => {
                      ...
                      setEventDate(d);
                      const busy = vendorsUnavailableOn(d);
                      if (busy.length) {
                        setErr(`${busy.map(v=>v.name).join(", ")} ${busy.length===1?"is":"are"} not available on ${d}. Pick another date, or remove them from your request.`);
                      } else { setErr(""); }
                    }}
                  ...
                  <div style={{ gridColumn:"1/-1", textAlign:"center", padding:"48px 20px" }}>
                    ...
                    <p style={{ margin:"6px 0 14px", fontSize:13, color:C.midGray, lineHeight:1.6 }}>
                      {(qWhere || qGuests || search)
                        ? "Try clearing your search criteria — or widen the location or guest count."
